...
class GenerateFeatures():
    ...
    def execute_model(self, model, feats_save_dir):
        ...
        for category in cat_list:
            image_list = glob.glob(
                self.config.image_dir + "/"+str(category)+"/*.jpg")
            image_list.sort()
    ...
    def get_model(self, model, load_model=None):
        # if self.config.load_model is not None and (self.config.fullblown or self.config.generate_features):
    ...
    def run(self):
        ...
        self.config.image_dir = os.path.join(
            self.config.data_dir, "images", "train_images")
